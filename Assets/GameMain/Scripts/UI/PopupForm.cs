...
{
    public class PopupForm : UGuiForm
    {
        [SerializeField]
...
#endif
        {
            ...
            PopupParams popupParams = (PopupParams)userData;
            if (popupParams == null)
            {
                Log.Warning("popupParams is invalid.");
                return;
            }

            m_DialogMode = popupParams.Mode;
            RefreshDialogMode();

            m_TitleText.text = popupParams.Title;
            m_MessageText.text = popupParams.Message;

            m_PauseGame = popupParams.PauseGame;
            RefreshPauseGame();

            m_UserData = popupParams.UserData;

            RefreshConfirmText(popupParams.ConfirmText);
            m_OnClickConfirm = popupParams.OnClickConfirm;

            RefreshCancelText(popupParams.CancelText);
            m_OnClickCancel = popupParams.OnClickCancel;

            RefreshOtherText(popupParams.OtherText);
            m_OnClickOther = popupParams.OnClickOther;
        }
